      ...
      <div className="container mx-auto px-2 sm:px-4">
        <div className="flex flex-col md:flex-row items-center justify-between py-3 gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className="text-left max-w-[220px] sm:max-w-none">
              <h1 className={`text-sm sm:text-lg md:text-xl lg:text-2xl font-bold leading-tight whitespace-nowrap overflow-hidden text-ellipsis ${lang === 'marathi' ? 'marathi-text' : 'english-text'}`}>
                {lang === 'marathi'
                  ? 'मोटार परिवहन विभाग, महाराष्ट्र राज्य पुणे'
                  : <>Motor Transport Department<br />Maharashtra State, Pune</>}
              </h1>
              <p className={`text-xs sm:text-sm text-muted-foreground whitespace-nowrap overflow-hidden text-ellipsis ${lang === 'marathi' ? 'marathi-text' : 'english-text'}`}>
                {lang === 'marathi'
                  ? 'महाराष्ट्र पोलीस अधिकृत संकेतस्थळ'
                  : 'Official website of Maharashtra Police'}
              </p>
            ...
            {navItems.map((item) => {
              if (item.key === "employee") {
                return (
                  <div key={item.key} className="relative">
                    <button
                      onClick={() => setEmployeeOpen(true)}
                      className={`police-nav-link flex items-center gap-1 whitespace-nowrap overflow-hidden text-ellipsis ${lang === 'marathi' ? 'marathi-text' : 'english-text'}`}
                    >
                      {getNavLabel(item.key)}
                      <ChevronDown className="w-4 h-4" />
                    </button>
                        ...
                        {sssServices.map((service, index) =>
                          service === "SSS" ? (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="block px-4 py-2 text-gray-700 hover:bg-blue-50 transition-colors rounded flex items-center justify-between whitespace-nowrap overflow-hidden text-ellipsis"
                            >
                              {service}
                              <ExternalLink className="w-4 h-4 text-gray-400" />
                            </a>
                            ...
                            <div
                              key={index}
                              className="block px-4 py-2 text-gray-500 flex items-center justify-between cursor-default whitespace-nowrap overflow-hidden text-ellipsis"
                            >
                              {service}
                            </div>
                          )
              ...
              return (
                <Link
                  key={item.key}
                  to={item.href}
                  className={`police-nav-link whitespace-nowrap overflow-hidden text-ellipsis ${lang === 'marathi' ? 'marathi-text' : 'english-text'}`}
                >
                  {getNavLabel(item.key)}
                </Link>
              );
                    ...
                    if (item.key === "employee") {
                      return (
                        <button
                          key={item.key}
                          onClick={() => setEmployeeOpen(true)}
                          className="police-nav-link text-base py-2 text-left whitespace-nowrap overflow-hidden text-ellipsis"
                        >
                          {getNavLabel(item.key)}
                        </button>
                      );
                      ...
                      return (
                        <div key={item.key} className="flex flex-col gap-1">
                          <button
                            ...
                            className="flex items-center justify-between px-4 py-2 font-semibold text-gray-600 border-b whitespace-nowrap overflow-hidden text-ellipsis"
                          >
                            {getNavLabel(item.key)}
                            <ChevronDown className={`w-4 h-4 transition-transform ${mobileSssOpen ? 'rotate-180' : ''}`} />
                          </button>
                              ...
                              {sssServices.map((service, idx) =>
                                service === "SSS" ? (
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="police-nav-link text-base py-2 flex items-center justify-between ml-4 hover:bg-blue-50 rounded transition-colors whitespace-nowrap overflow-hidden text-ellipsis"
                                  >
                                    {service}
                                    <ExternalLink className="w-4 h-4 text-gray-400" />
                                  </a>
                                  ...
                                  <div
                                    key={idx}
                                    className="police-nav-link text-base py-2 flex items-center justify-between ml-4 text-gray-500 cursor-default whitespace-nowrap overflow-hidden text-ellipsis"
                                  >
                                    {service}
                                  </div>
                                )
                    ...
                    if (item.external) {
                      return (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="police-nav-link text-base py-2 flex items-center gap-1 whitespace-nowrap overflow-hidden text-ellipsis"
                        >
                          {getNavLabel(item.key)}
                          <ExternalLink className="w-4 h-4 text-gray-400" />
                        </a>
...
=======
                        className="police-nav-link text-base py-2 whitespace-nowrap overflow-hidden text-ellipsis"
>>>>>>> 7efdc98 (Last  Update)
                      >
                        {getNavLabel(item.key)}
                      </Link>
